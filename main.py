...
def parse_rows_from_request(request, key, delimiter=',', 
                            base_rows=1, max_row=9, max_column=23):
  """
  Returns dictionary from a request which holds values of the form
  "%s_column%s" % (key, i) and "key_rows%s" % (key, i) for
  ...
  while column is not None:
    ...
    if rows != '':
      # this includes column is not None, rows is None
      for row in rows.split(delimiter):
        try:
          row = int(row)
...
class Play(webapp.RequestHandler):
  ...
  def get(self):
    ...
    sprite_purchased = PurchasedItem.gql(("WHERE federated_identity = '%s' "
                                         ...
                                         (user.federated_identity(),
                                          'Sprite'))
    if sprite_purchased.count():
      sprite = 'translate_robot-lb64'
    else:
      sprite = 'android-%s' % curr_level.entity_size
...
class BuildLevel(webapp.RequestHandler):
  ...
  def send_error(self, message, can_build):
    template_vals = {'can_build': can_build,
                     'message': message,
                     'error': True}
    path = os.path.join(os.path.dirname(__file__),
  ...
  def post(self):
    ...
    if level_name in ['1', '2', '3', '4', '5']:
      self.send_error('%s is already a level' % level_name, can_build)
      return
    else:
    ...
    try:
      ...
      if base_rows < 0 or base_rows > max_row:
        self.send_error(('%s is invalid for rows at bottom '
    ...
    # Validate static #
    #####################
    static = parse_rows_from_request(self.request, 'static')
    if static is None:
      self.send_error(('Static Blocks invalid. Please use '
    ...
    # Validate moveable #
    #####################
    moveable = parse_rows_from_request(self.request, 'moveable')
    if moveable is None:
      self.send_error(('Moveable Blocks invalid. Please use '
                       'integers and separate rows by commas'), can_build)
      return
    ...
    for column, row_list in moveable.iteritems():
      ...
      for row in row_list:
        if row in destination:
          self.send_error(('Moveable and Static Blocks can\'t occupy '
          ...
          if row - 1 not in (row_list + destination):
            self.send_error('Moveable can\'t hover', can_build)
            return
    ...
    try:
      ...
      if (door_row < base_rows or door_row > max_row - 1 or
        ...
        self.send_error('Door out of bounds', can_build)
        return
      ...
      if door_column in static:
        if (door_row in static[door_column] or
          ...
          self.send_error('Door conflicts with Static Blocks', can_build)
          return
      elif door_column in moveable:
        if (door_row in moveable[door_column] or
            door_row + 1 in moveable[door_column]):
          self.send_error('Door conflicts with Moveable Blocks', can_build)
          return
    except ValueError, TypeError:
      self.send_error('Door values invalid', can_build)
      return
    ...
    try:
      ...
      if (player_row < base_rows or player_row > max_row or
        ...
        self.send_error('Player out of bounds', can_build)
        return
      ...
      if player_column in static and player_row in static[player_column]:
        self.send_error('Player conflicts with Static Blocks', can_build)
        return
      elif player_column in moveable and player_row in moveable[player_column]:
        self.send_error('Player conflicts with Moveable Blocks', can_build)
        return
      ...
      if player_row != base_rows:
        ...
        if not block_below:
          self.send_error('Player must start grounded', can_build)
          return
    except ValueError, TypeError:
      self.send_error('Player values invalid', can_build)
      return
    ...
    new_level = Level(owner=user,
                      level=level_name,
                      base_rows=base_rows,
                      static_blocks=db_pickle(static),
                      move_blocks=db_pickle(moveable),
                      door=db_pickle(door),
                      player_start=db_pickle(player_start))
    new_level.put()
